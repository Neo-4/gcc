...
/* 从cl_options[]列表里面，匹配命令行传入的参数，将参数在cl_options[]列表中的索引号返回。*/
size_t
find_opt (const char *input, unsigned int lang_mask)
{
  ...
  /*
    这里采用二分法来查找cl_options列表中匹配传入的参数。cl_options中的参数，必须按照ASCII表上的顺序排放。
    当mx-mn=0时，说明查找到匹配的参数。
  */
  while (mx - mn > 1)
  {
    md = (mn + mx) / 2;
    ...
    /* strncmp函数为字符串比较函数，字符串大小的比较是以ASCII 码表上的顺序来决定，
        此顺序亦为字符的值。该函数对str1和str2进行比较，最多比较前n个字节。
        若str1与str2的前n个字符相同，则返回0
        若s1大于s2，则返回大于0的值
        若s1 小于s2，则返回小于0的值
        例子：strncmp("-abc", "-abe", 4)
        那么分别用'-'-'-'==0;'a'-'a'==0;'b'-'b'==0;'c'-'e'==2,此时不为0，那么返回2
    */
		       ...
		       struct cl_decoded_option *decoded)
{
  ...
  /* 取参数-后面的内容，比如:./gcc -o main.c中的o
    然后从列表cl_options[]中匹配对应的参数，并把该参数在cl_options[]列表中的索引号返回。
  */
  ...
  /*
    因为有些命令在find_opt中不能被直接识别，需要将这些命令转成find_opt能识别的参数。
    下面这些左边的参数，需要转成右边的参数，再给find_opt识别：
    -Wno-xxxxx：-Wxxxxx
    -fno-xxxxx: -fxxxxx
    -mno-xxxxx：-mxxxxx
    --debug=xxxxx：-gxxxxx
  ...
  */
  i = 0;
  while (opt_index == OPT_SPECIAL_unknown && i < ARRAY_SIZE (option_map))
  {
      const char *opt0 = option_map[i].opt0;
  ...
  if (opt_index == OPT_SPECIAL_unknown)
 {
  ...
  if (option->alias_target != N_OPTS && (!option->cl_separate_alias || have_separate_arg))
  {
      ...
      else
	  {
	    ...
	    gcc_assert (new_option->alias_target == N_OPTS || new_option->cl_separate_alias);
  ...
  else if (strcmp (option->opt_text, "-Werror=") == 0 && strchr (opt_value, ',') == NULL)
  {
	  ...
	  {
	    const struct cl_option *warning_option = &cl_options[warning_index];
				 ...
				 unsigned int *decoded_options_count)
{
  ...
  // 每一个命令行参数，都分得一个cl_decoded_option结构体。
  ...
  // 打印所有gcc支持的参数
//  FILE *file;
//  file = fopen("out_gcc_options.txt", "w");
...
//  fclose(file);
  ...
  for (i = 1; i < argc; i += n)
  {
    const char *opt = argv[i];
        ...
        // 如果参数不带有-开头的，都认为是文件。比如./gcc main.c中的main.c
    if (opt[0] != '-' || opt[1] == '\0')
    {
    ...
    // 处理带有-开头的gcc参数，比如./gcc -o main.c中的-o
    n = decode_cmdline_option (argv + i, lang_mask, &opt_array[num_decoded_options]);
    num_decoded_options++;
  }
